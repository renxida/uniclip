...
class Server:
    ...
    def handle_register(self):
        ...
        self.logger.debug(f"Received registration request for group: {group_id}")
    ...
    def handle_update(self):
        ...
        self.logger.debug(f"Received update request from {sender_ip} for group: {group_id}")
    ...
    def handle_poll(self, group_id):
        ...
        self.logger.debug(f"Received poll request from {client_ip} for group: {group_id}")
        ...
        if group_id in self.pending_updates and client_ip in self.pending_updates[group_id]:
            ...
            self.logger.info(f"Sending update to client {client_ip} in group {group_id}")
            ...
            self.logger.debug(f"No pending updates for client {client_ip} in group {group_id}")
